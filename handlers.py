...
async def start_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Вітаю! Мене звати FilmButler і я твій персональний помічник у пошуку фільмів! Для початку обери один із варіантів нижче \U0000263A",
...
async def message_handler(message: types.Message, state: FSMContext):
    if message.text == "Шукати фільм за назвою \U0001F50D":
        ...
        await message.answer("Звісно! Напиши, будь-ласка, назву фільму котрий хочеш знайти.",
        ...
        await message.answer(
            "Добре, надай назву будь-якого фільму і я створю для тебе список зі схожими кінострічками.",
            reply_markup=types.ReplyKeyboardRemove())
    else:
        current_state = await state.get_state()
        if current_state == Form.normal_search:
            await search_movies(message, state)
        ...
        async with session.get(url) as response:
            ...
            if data['results']:
                ...
                for film in data['results']:
                    title = film['title']
                    release_date = film['release_date'][:4] if 'release_date' in film and film[
                        'release_date'] else 'N/A'
                    if current_state == Form.normal_search:
                        inline_keyboard.append([InlineKeyboardButton(text=f"{title} - {release_date}",
                                                                     callback_data=f"film_info:{film['id']}")])
                    elif current_state == Form.recommendation:
                        inline_keyboard.append([InlineKeyboardButton(text=f"{title} - {release_date}",
                                                                     callback_data=f"recommend:{film['id']}")])
                reply_markup = InlineKeyboardMarkup(inline_keyboard=inline_keyboard)
                await message.answer("Ось результати пошуку. Будь-ласка, обери фільм котрий маєш наувазі:",
                                     reply_markup=reply_markup)
            else:
                await message.answer("Вибачте, але я не зміг знайти фільм за наданою назвою, спробуйте іншу \U0001F625",
                                     reply_markup=main_menu_buttons())
            ...
            if data['results']:
                ...
                for movie in data['results']:
                    inline_keyboard.append(
                        [InlineKeyboardButton(text=movie['title'], callback_data=f"film_info:{movie['id']}")])
                reply_markup = InlineKeyboardMarkup(inline_keyboard=inline_keyboard)
...
async def show_film_info(callback_query: CallbackQuery):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(url) as response:
            ...
            # Якщо трейлеру українською немає, додаю трейлер англійською
            if not trailer_url:
            ...
            # перевірка на працездатність посилання
            if poster_url:
                try:
...
async def show_recommendations(callback_query: CallbackQuery):
    film_id = callback_query.data.split(":")[1]
    # видаляю попередній список з уточненням фільму
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(recommendations_url) as rec_response:
            ...
            if rec_data['results']:
                # парсинг назви у окрему змінну для використання у повідомленні з наданим списком
                ...
                for rec_film in rec_data['results']:
                    inline_keyboard.append(
                        [InlineKeyboardButton(text=rec_film['title'], callback_data=f"film_info:{rec_film['id']}")])
                reply_markup = InlineKeyboardMarkup(inline_keyboard=inline_keyboard)
                await callback_query.message.answer(f"Якщо тобі подобається {title}, я рекомендую подивитись:",
                                                    reply_markup=reply_markup)
            else:
                await callback_query.message.answer("Вибач, але я не зміг створити список рекомендацій за цим фільмом.",
                                                    reply_markup=main_menu_buttons())
